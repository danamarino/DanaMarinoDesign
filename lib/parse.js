...
files.forEach(file => {
    ...
    const keywordsTag = dom.window.document.querySelector('meta[name="keywords"]');

    const pageKeywords = keywordsTag ? keywordsTag.getAttribute('content') : '';
    ...
    const outFile = path.basename(file).replace('html', 'page.json');
    const outputPath = `${outDir}/${outFile}`;

    console.info(`Create: ${outputPath}`)

    writeFileSync(outputPath, JSON.stringify({
        template: `${outDir}/${path.basename(file)}`,
        head,
        body
    }, null, 4), 'utf8');
});
